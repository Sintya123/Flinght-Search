...
import com.example.flightsearch.data.model.FlightSearchRepository
import com.example.flightsearch.data.model.OfflineRepository
    ...
    fun isFavorite(departureCode: String, destinationCode: String): Boolean {
        if (favoriteUi.favorites.isNotEmpty()) {
            favoriteUi.favorites.forEach { favorite ->
                if (favorite.departureAirport.iata_code == departureCode && favorite.destinationAirport.iata_code == destinationCode) return true
            }
        }
        return false
    }
